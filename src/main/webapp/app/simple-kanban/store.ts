...
const copyOrInsertCard = (state: BoardState, clonedCard: Card): BoardState => {
  ...
  if (state.cardInFlight) {
    ...
    if (state.isCopying) {
      nextId++;
      board = state.spilledCard ? state.board : insertCard(state.board, clonedCard, hover.listId, hover.index);
    } else {
...
export function cardReducer(state: BoardState | any, action: SortCard) {
  const { data, index, listId, hover } = action.item;
  ...
  switch (action.event) {
    ...
    case SortableEvents.Drop: {
      if (state.cardInFlight) return copyOrInsertCard(state, cloneCard(state.cardInFlight.data, state.nextId));
      return;
    }
    case SortableEvents.EndDrag: {
      return resetDrag(state);
...
export function reducer(state: BoardState = initialState, action: Actions): BoardState {
  ...
  switch (action.type) {
    ...
    case ActionTypes.AddCard: {
      ...
      const list: any = currentBoard.find(x => x.id === action.listId);
...
export const _listById = (listId: any) =>
  createSelector(_board, board => {
    const list: any = board.find(l => l.id === listId);
...
export const _render = createSelector(_boardState, _temporaryClone, (state, tempClone: any) => {
