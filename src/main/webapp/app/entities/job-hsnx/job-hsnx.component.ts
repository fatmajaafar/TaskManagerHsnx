...
export class JobHsnxComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.jobService
        ...
        .subscribe(
          (res: HttpResponse<IJobHsnx[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  trackId(index: number, item: IJobHsnx): number {
  ...
  delete(job: IJobHsnx): void {
    const modalRef = this.modalService.open(JobHsnxDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IJobHsnx[] | null, headers: HttpHeaders, page: number): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    this.router.navigate(['/job-hsnx'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
